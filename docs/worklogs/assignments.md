...
| BC-041-C1 | Unicode Format-Character OPSEC Correction | Codex | review | `docs/domains/runtime/assignments/BC-041-C1/assignment.md` | `33b44608cb634d1fedeed7f5f70d405c3999ed02` | B-1 corrected with dual deterministic `Cf` candidate views; 24-case ingress/egress matrix and existing negative regressions pass; N-2 review state is `required_pending`; N-7 homoglyph limitation explicit; substantive SHA recorded by metadata receipt; independent Claude review pending; no runtime, provider, Auth, architecture, or production-policy work |
